...
def _prompt_int(prompt: str, default: int = 1, choices: tuple[int, ...] = (1, 2)):
    while True:
...
def _split_groups(tokens: list[str]):
...
def _parse_set_args(tokens: list[str]):
    ...
    ns: argparse.Namespace = sp.parse_args(tokens)
...
def main():
    ...
    if not rest:
        ...
        fig, _ax = run_visualization(
            ...
            alpha=alpha,
            tick_step=tick_step,
            grid_res=max(3, int(args.grid_res)),  # защита от слишком маленького значения
        )
        if args.save:
