...
def run_visualization(
    ...
    # ===== 1) Сетка: оверскан на одну ячейку с каждой стороны =====
    ...
    mask_nodes = clip_path.contains_points(grid_points, radius=1e-12).reshape(
        grid_x.shape
    )
    ...
    # ===== 5) Границы Z кратно шагу =====
    zmin_surface = (
        np.nanmin(Z1) if Z2 is None else float(min(np.nanmin(Z1), np.nanmin(Z2)))
    )
    zmax_surface = (
        np.nanmax(Z1) if Z2 is None else float(max(np.nanmax(Z1), np.nanmax(Z2)))
    )
    ...
    # ===== 6) Палитра / дискретная нормализация =====
    ...
    # ===== 7) Пер-лицевые цвета (ячейки), чтобы не рисовать «половинки» =====
    def make_facecolors(Z: np.ndarray) -> np.ndarray:
        ...
        cell_mask = (
            cell_inside & cell_finite
        )  # ячейка валидна, если ВСЕ 4 угла внутри и не NaN
        ...
        ring_nodes = mask_nodes & ~binary_erosion(
            mask_nodes, structure=np.ones((3, 3), dtype=bool)
        )
        ring_cells = (
            ring_nodes[:-1, :-1]
            | ring_nodes[1:, :-1]
        ...
        fc[ring_cells & cell_mask, 3] = np.minimum(
            fc[ring_cells & cell_mask, 3], alpha * 0.35
        )
        return fc
    ...
    # ===== 9) Поверхности (пер-лицевые цвета) =====
    ...
    # ===== 10) Подписи точек, как у вас (оставил без изменений, с плашками) =====
    val_dec = 2 if step < 0.1 else 1
    val_fmt = f"{{:.{val_dec}f}}"
    box1 = dict(
        boxstyle="round,pad=0.15,rounding_size=0.10", fc="white", lw=0.8, alpha=0.75
    )
    box2 = dict(
        boxstyle="round,pad=0.15,rounding_size=0.10", fc="#f3f3f3", lw=0.8, alpha=0.75
    )
    ...
    for label, p in pts1_ordered:
        ...
        txt.set_path_effects(
            [pe.withStroke(linewidth=2.0, foreground="white", alpha=0.8)]
        )
    ...
    cbar = fig.colorbar(
        sm, ax=ax, shrink=0.6, aspect=10, pad=0.05, boundaries=levels, ticks=levels
    )
